...
file.names <- c("supp_simulation_weakconf.RData")
...
a.obj <- simulation.list$`supp_simulation_weakconf.RData`$Predictions

# Separate as Misspecified + Correctly Specified Models
sim_bivar <- simulation
true.rep.arr <- array(NA, dim = c(grid.grid.num.d, grid.grid.num.g, 1, nrow(sim_bivar$Brier)))
...
library(stringi)
rename.files <- T
dir.spec <- here(res.dir, "To Present", "2023_11_30", "Correct")
if(rename.files) {
