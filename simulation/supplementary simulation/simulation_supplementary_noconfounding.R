library(here)
res.dir <- here("results", "Manuscript", "Exploratory",
                "Multiple Replicates")
...
source(here("main simulation", "true_risk_noconf.R"))
...
meanfunc.sim <- function(num.cores = 8, seeding = 1) {
  ...
  para.sim <- function(core.no) {
    ...
    out.parms <- c(-18, 0.45, 0, 0) # c(-18, 0.45, 3, 3)
...
# Pointwise Causal Risks
# Logistic, Polynomial Logistic, Additive (Doubly), Bivariable (200 MCMC Iterations, n = 100)
# Start Time @ 2023-12-02 10:54:51 EST
(sim.begin <- Sys.time())
simulation <- meanfunc.sim(num.cores = 8, seeding = 1)
(simulation$time <- Sys.time() - sim.begin)    # Total Simulation Time
# save(simulation, file = file.path(outpath2, paste0("supp_simulation_noconf.RData")))


# Notes:
# n = 500 - 4 hours (2 rounds / 16 replicates); 21 hours (11 rounds / 88 replicates)