...
source(here("main simulation", "true_risk_strongconf.R"))
...
meanfunc.sim <- function(num.cores = 8, seeding = 1) {
  ...
  para.sim <- function(core.no) {
    ...
    out.parms <- c(-21, 0.5, 1, 3) # c(-18, 0.45, 3, 3)
...
# save(simulation, file = file.path(outpath2, paste0("supp_simulation_strongconf.RData")))