...
# Estimated Run Time: 17.1 hours (1000 resamples using 8 cores)
...
meanfunc.boot <- function(num.cores = 8, seeding = 1) {
  ...
  para.sim <- function(core.no) {
    ...
    q_bladder <- 0.3
    d.star_bladder <- 40
    d.star_bladder <- dvals[findInterval(d.star_bladder, dvals)]
    ...
    for(r in 1:R) {
      ...
      boot.d.star.data_bladder <- boot.long[which(boot.long$Dose == d.star_bladder),]
      
      # Outcome
      y.out <- boot.long$`GU Toxicity`
      ...
      vol.ind <- boot.long$Volume_Bladder
      ...
      cov.dstar <- d.star.data_bladder$Age65_numeric
      ...
      obs.g.0 <- boot.d.star.data_bladder$Volume_Bladder[cov.dstar == 0 & boot.d.star.data_bladder$Volume_Bladder <= q_bladder]
      obs.g.1 <- boot.d.star.data_bladder$Volume_Bladder[cov.dstar == 1 & boot.d.star.data_bladder$Volume_Bladder <= q_bladder]
      ...
      for(z in 1:mod.size) {
        ...
        cond.means <- data.frame(
          `g` = boot.long$Volume_Bladder[which(boot.long[,"Dose"] == d.star_bladder)],
          `risk`= mod.preds.train[,z][which(boot.long[,"Dose"] == d.star_bladder)]
        ) %>% cbind(boot.long[which(boot.long[,"Dose"] == d.star_bladder), cov.labs])
...
# Pararellization: Pointwise & Stochastic Causal Risks
# (sim.begin <- Sys.time())
