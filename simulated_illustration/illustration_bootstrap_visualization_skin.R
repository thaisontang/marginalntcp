...
load(file = file.path(outpath2, paste0("ntcp_bootresamp_age65_skin.RData")))
...
dvh.bound <- long.data %>% group_by(Dose) %>%
  summarise(`minVol` = signif.floor(min(Volume_Skin), 2),
            `maxVol` = signif.ceiling(max(Volume_Skin), 2)) %>%
...
poly.seq <- long.data %>% group_by(Dose) %>%
  summarise(`minVol` = signif.floor(min(Volume_Skin), 2),
            `maxVol` = signif.ceiling(max(Volume_Skin), 2) + 0.02)
...
for(j in 1:num.pars) {
  ...
  pdf(here(res.dir, paste0("Figure3", letters[j],".pdf")),
      width = 10, height = 6)
