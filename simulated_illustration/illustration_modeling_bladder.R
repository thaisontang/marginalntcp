...
vol.ind <- long.data$Volume_Bladder
...
obs.g.0 <- d.star.data_bladder$Volume_Bladder[cov.dstar == 0 & d.star.data_bladder$Volume_Bladder <= q_bladder]
obs.g.1 <- d.star.data_bladder$Volume_Bladder[cov.dstar == 1 & d.star.data_bladder$Volume_Bladder <= q_bladder]
...
for(z in 1:mod.size) {
  ...
  cond.means <- data.frame(
    `g` = long.data$Volume_Bladder[which(long.data[,"Dose"] == d.star_bladder)],
    `risk`= mod.preds.train[,z][which(long.data[,"Dose"] == d.star_bladder)]
  ) %>% cbind(long.data[which(long.data[,"Dose"] == d.star_bladder), cov.labs])
...
for(z in 1:mod.size) {
  ...
  cond.means <- data.frame(
    `g` = long.data$Volume_Bladder[which(long.data[,"Dose"] == d.star_bladder)],
    `risk`= mod.preds.train.unadj[,z][which(long.data[,"Dose"] == d.star_bladder)]
  )
  
  # P(Y = 1 | d^*, g, x) relevant to Stochastic Intervention
  # The code below is for the special case of a binary covariate
  est.risks <- cond.means$risk[cond.means$g <= q_bladder]
  
  # Importance Sampling Weight
  est.isws <- 1/length(d.star.data_bladder$Volume_Bladder[d.star.data_bladder$Volume_Bladder <= q_bladder])
